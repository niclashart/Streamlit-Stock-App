...
def get_portfolio(user_id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT ticker, shares, entry_price, purchase_date FROM portfolios WHERE user_id = ?", (user_id,))
    portfolio_data = cursor.fetchall()
    conn.close()
    # Convert to DataFrame to maintain compatibility with existing app logic expecting DataFrames
    df = pd.DataFrame(portfolio_data, columns=["Ticker", "Anteile", "Einstiegspreis", "Kaufdatum"])
...
def add_to_portfolio(user_id, ticker, shares, entry_price, purchase_date):
    ...
    if existing_position:
        ...
        cursor.execute("UPDATE portfolios SET shares = ? WHERE id = ?", (new_shares, existing_position['id']))
    else:
        cursor.execute("""
...
def update_portfolio_after_sell(user_id, ticker, quantity_to_sell):
    ...
    for position in positions:
        ...
        if position_shares > remaining_quantity_to_sell:
            ...
            cursor.execute("UPDATE portfolios SET shares = ? WHERE id = ?", (new_shares, position_id))
...
def get_orders(user_id=None, status=None):
    ...
    df_columns = ["id", "username", "ticker", "order_type", "price", "quantity", "created_at", "status"]
